...
function CheckCurrentClothes(gender, skin1, skin2, skin3, skin4, skin5, skin6)
	...
	for k,v in pairs(HasItem) do
		if v.metadata.gender == gender and v.metadata.torso1 == skin1 and v.metadata.torso2 == skin2 and v.metadata.arms1 == skin3 and v.metadata.arms2 == skin4 and v.metadata.tshirt1 == skin5 and v.metadata.tshirt2 == skin6 then
			hasClothes = false
			TriggerClientEvent('col_clothes:Notification', source, {id = 'error_have_item', title = 'SYSTEM', description = 'You already have a item clothes. please check your inventory', duration = 5000, position = 'top', style = {backgroundColor = '#FF4B4E', color = '#FFFFFF'}, icon = 'ban', iconColor = '#FFFFFF'})
		else
			hasClothes = true
		end
	end	
...
function CheckCurrentShits(gender, skin1, skin2, theType)
	...
	for k,v in pairs(HasItem) do
		if v.metadata.gender == gender and v.metadata.accessories == skin1 and v.metadata.accessories2 == skin2 then
			hasValid = false
			TriggerClientEvent('col_clothes:Notification', _source, {id = 'error_have_item', title = 'SYSTEM', description = 'You already have a item clothes. please check your inventory', duration = 5000, position = 'top', style = {backgroundColor = '#FF4B4E', color = '#FFFFFF'}, icon = 'ban', iconColor = '#FFFFFF'})
		else
			hasValid = true
		end
	end	
